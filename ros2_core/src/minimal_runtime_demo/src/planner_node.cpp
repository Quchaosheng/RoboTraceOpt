...
public:
  ...
  : Node("planner_node")
  {
    planner_delay_ms_ = this->declare_parameter<int>("planner_delay_ms", 50);
...
private:
  ...
  void handle_input(const std_msgs::msg::String::SharedPtr msg)
  {
    ...
    if (!payload.has_value()) {
      RCLCPP_WARN(this->get_logger(), "Failed to parse /demo/input payload: %s", msg->data.c_str());
      return;
    }

    publish_event(*payload, "planner_receive");
    publish_event(*payload, "planner_process_start");
    std::this_thread::sleep_for(std::chrono::milliseconds(planner_delay_ms_));
    publish_event(*payload, "planner_process_end");
    publish_event(*payload, "planner_publish");
