...
if [[ "$(id -u)" -ne 0 ]]; then
  echo "Run with sudo: sudo bash scripts/run_native_x86_2404_f3_f4.sh" >&2
  exit 2
fi
...
RUN_ID="native_x86_2404_jazzy_f3f4_$(date -u +%Y%m%d_%H%M%S)"
...
echo
echo "F3/F4 Ubuntu 24.04/Jazzy session completed: ${OUTPUT_ROOT}"
echo "Copy this directory back together with: ${CAPABILITY_PATH}"
